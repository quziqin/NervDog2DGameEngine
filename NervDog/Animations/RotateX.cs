...
{
    public class RotateX : Animation
    {
        ...
        public RotateX(float angleFrom, float angleTo, uint duration)
        {
        ...
        protected override bool UpdateFrame()
        {
            if (_reverse)
            {
                if (_isReversing)
                {
                    if (_current >= 0.0f)
                    {
                        _target.RotateX = _ease.Func(_current, _angleFrom, _angleChange, _count);
                        _current -= _delta;
                    }
                    else
                    {
